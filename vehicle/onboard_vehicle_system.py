...
class OnboardVehicleSystem:
    ...
    def broadcast_telem(self):
        ...
        while not self.kill.kill:
            ...
            tic = time.time()
            if self.location_data == "gps":
                self.lon, self.lat, self.alt = self.gps.getTelemetry()
            else:
                self.lon, self.lat, self.alt = self.update_uav_gps()
            ...
            try:
                print("Sending Telemetry Data (%s:%s)" % (self.lat, self.lon))
                self.telem_broadcast_sock.sendto(json.dumps(_msg).encode("utf-8"), (TELEM_IP, TELEM_PORT))
            except:
                pass
            toc = time.time() - tic
            try:
