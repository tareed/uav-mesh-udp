...
class GroundControl:
    ...
    def __init__(self, onesky_api, host_ip, silvus_ip, killer):
        ...
        ## self.gcs_recv_telem_sock.setsockopt(socket.SOL_IP, socket.IP_MUTLICAST_IF, socket.inet_aton(host))
    ...
    def listen(self):
        ...
        while not self.kill.kill:
            try:
                ...
                #print("SOCKET DATA: %s" % ((_data).decode("utf-8")))
    ...
    def init_flight(self, new_flight, addr):
        ...
        gufi = str(uuid.uuid1()) #self.onesky.createPointFlight(new_flight["name"], new_flight["lon"], new_flight["lat"], new_flight["alt"])       
        with self.agent_lock:
